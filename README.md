...
- Telegraph page for large search results (keeps chat clean)
- Streamable Telegram video uploads (with duration and optional custom thumbnail)
- Automatic multi-part upload to Telegram for files larger than 2 GB (configurable part size)
- Inline, owner-only settings menu to tweak runtime bot settings
...
### Telegram upload options
- **UPLOAD_AS_VIDEO**: (Optional) `true`/`false`. When true and file is `.mp4`/`.mkv`, upload as streamable video to Telegram (adds duration and supports streaming). Otherwise uploads as a document.
- **VIDEO_THUMB_PATH**: (Optional) Absolute path to an image to use as default video thumbnail.
- **USE_CUSTOM_THUMB**: (Optional) `true`/`false`. When true and `VIDEO_THUMB_PATH` is set, use that thumbnail for video uploads.
- **TG_PART_SIZE_MB**: (Optional) Size in MB for each Telegram upload part when files exceed the 2 GB limit. Default `1950`. Keep below 2000 to avoid hitting Telegram limits.
...
# Commands
- `/settings` (owner only): open inline settings to toggle Team Drive, Service Accounts, status update interval, auto-delete behavior, upload-as-video, and custom thumbnail usage.

# Notes
- Large search results are posted to a Telegraph page automatically; the bot replies with a Telegraph link instead of spamming the chat.
- For Telegram files exceeding 2 GB, the bot splits and uploads parts sequentially; part size is configurable via `TG_PART_SIZE_MB`.
