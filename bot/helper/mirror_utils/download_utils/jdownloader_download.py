...
class JDownloaderDownloader(DownloadHelper):
    """
    JDownloader download helper for MEGA and other supported links
    Uses JDownloader's REST API to manage downloads
    """
    ...
    def __init__(self, listener):
        ...
        # Try to get JDownloader config from environment
        self.__load_jd_config()
        
    def __load_jd_config(self):
        """Load JDownloader configuration from environment variables"""
        import os
    ...
    def __get_device_id(self):
        ...
        try:
            ...
            if response and 'data' in response:
                # Try to get device ID from downloads endpoint
    ...
    def __add_download_to_jd(self, link, path):
        ...
        try:
            ...
            if response and 'data' in response:
                ...
                LOGGER.info(f"Added download to JDownloader with package ID: {package_id}")
                return package_id
            else:
                raise Exception("Failed to add download to JDownloader")
                
        except Exception as e:
            LOGGER.error(f"Failed to add download to JDownloader: {e}")
            raise
    ...
    def __onInterval(self):
        ...
        try:
            ...
            if self.__status in ['FINISHED', 'COMPLETED']:
                ...
                error_msg = status_data.get('errorMessage', 'Download failed')
                self.__onDownloadError(error_msg)
                return
            elif self.__status in ['CANCELLED', 'STOPPED']:
                self.__onDownloadError('Download cancelled')
                return
    ...
    def add_download(self, link, path):
        ...
        try:
            ...
            # Add download to JDownloader
            package_id = self.__add_download_to_jd(link, path)
            
            # Get initial download info
            status_data = self.__get_download_status(package_id)
            if status_data:
                file_name = status_data.get('name', Path(path).name)
                file_size = status_data.get('bytesTotal', 0)
                
                self.__onDownloadStart(file_name, file_size, package_id)
                LOGGER.info(f'Started JDownloader download with package ID: {package_id}')
            else:
                raise Exception("Failed to get download information")
                
        except Exception as e:
            LOGGER.error(f'Failed to start JDownloader download: {e}')
    ...
    def cancel_download(self):
        ...
        try:
            ...
            if device_id:
                ...
                LOGGER.info(f'Cancelled JDownloader download: {self.__gid}')
