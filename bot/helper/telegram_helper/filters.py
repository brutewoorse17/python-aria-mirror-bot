...
class CustomFilters:
    ...
    def _authorized_user_filter(update):
    ...
    authorized_user = filters.UpdateType.MESSAGE & filters.Create(_authorized_user_filter)

    def _authorized_chat_filter(update):
        chat = update.effective_chat
        return bool(chat and chat.id in AUTHORIZED_CHATS)

    authorized_chat = filters.UpdateType.MESSAGE & filters.Create(_authorized_chat_filter)

    def _mirror_owner_filter(update):
    ...
    mirror_owner_filter = filters.UpdateType.MESSAGE & filters.Create(_mirror_owner_filter)
